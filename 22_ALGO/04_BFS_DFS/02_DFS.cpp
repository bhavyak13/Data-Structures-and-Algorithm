...
/*-----------------begin---------------*/
void DFS(int ind,int*explored,vector<vector<int>>graph){
    if(explored[ind])return;
    explored[ind]=1;
    pt(ind);
    vfor(graph[ind]){
        if(explored[*i]==0)DFS(*i,explored,graph);
    }
}
void solve()
{
    ...
    //     int x=stk.top();
    //     stk.pop();
    //     if(explored[x]==1)continue;
    //     else{explored[x]=1;}
    //     pt(x);
    //     vfor(graph[x]){
    //         int d=*i;
    //         if(!explored[d])stk.push(*i);
    //     }
    // }
}
/*-----------------end---------------*/
