...
bool isSubsetSum(int n, int arr[], int sum)
{
    ...
    ffor(i, 1, n + 1)
    {
        ffor(j, 1, sum + 1)
        {
            if(dp[i-1][j]==1)dp[i][j] = dp[i - 1][j]; //copying prev. value
            //or dp[i][j] = dp[i - 1][j] || dp[i][j];
            if (arr[i - 1] <= sum)
            {
                if (j == arr[i - 1]) dp[i][j] = 1; //marking curr array value as 1
                if (dp[i - 1][j] == 1 && (arr[i - 1] + j) <= sum)
                    dp[i][arr[i - 1] + j] = 1;
            }
        }
    }
