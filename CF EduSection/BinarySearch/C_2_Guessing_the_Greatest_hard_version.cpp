...
{
    in(n);
    int secondMx=query(1,n);
    if(secondMx!=n&&query(secondMx,n)==secondMx){
        int l=secondMx,h=n;// we can take l as secondMx bcs econdMx can never be our ans!
        while(h-l>1){
            int mid=(l+h)>>1;
            int q=query(secondMx,mid);
            if(q==secondMx)h=mid;
            else l=mid;
        }
        pns(h);
