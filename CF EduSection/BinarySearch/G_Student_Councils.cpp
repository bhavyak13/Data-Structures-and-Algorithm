...
vi a;
int k;
bool ok(int numCouncil){
    int req=k*numCouncil;
    for(auto i:a){
        req-=min(i,numCouncil);//minimum of students in the group,number of councils that student from ith group can be part of
    }
    return req<=0;
}
...
void solve()
{
    ...
    a.resize(n);
    cin>>a;
    // bs on num of councils that can be formed
    int l=-1,r=1;
    while(ok(r))r=r<<1;
    while(r-l>1){
        int mid=(l+r)>>1;
