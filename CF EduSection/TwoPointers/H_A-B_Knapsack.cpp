...
void solve()
{
    ...
    ffor(i,1,n+1)preA[i]=preA[i-1]+a[i-1];
    ffor(i,1,m+1)preB[i]=preB[i-1]+b[i-1];
    if(wtA*n+wtB*m<=k){// k/capacity is greater than overall weight.
       pn(preA[n]+preB[m]);
       return;
    }
    int ans=0;
    // k = 23
    // 8 15 20 24 27 28 
    // (12 22 31 39) 46 53 56
    int indx=min(m,k/wtB);//23/5=4;
    while(indx>=0){// 4 3 2 1 0
        int rem=k-wtB*indx;//23-5*4=3
        int Aindx=rem/wtA;//4/3=1
        if(Aindx>n)break;
        ans=max(ans,preB[indx]+preA[Aindx]);
        indx--;
    }
    indx=min(n,k/wtA);//23/3=(6,7)=6;
    while(indx>=0){// 6 5 4 3 2 1 0
        int rem=k-wtA*indx;//23-3*6=5
        int Bindx=rem/wtB;//5/5=1
        if(Bindx>m)break;
        ans=max(ans,preB[Bindx]+preA[indx]);
        indx--;
    }
    pn(ans);
