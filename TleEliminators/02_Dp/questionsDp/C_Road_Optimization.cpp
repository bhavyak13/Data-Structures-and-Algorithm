...
auto fun(){}
// dp[i][k] -> standing at i th checkpost
// and we are bound to pick this check post
// exaclty k signs are allowed for removal from this point
// wht the minimum time to reach end
void solve()
{
    in3(n,l,k);
    vi coordinate(n+1),speed(n+1);
    ffor(i,0,n)cin>>coordinate[i];
    ffor(i,0,n)cin>>speed[i];
    coordinate[n]=l;
    speed[n]=0;
    vvi dp(n+1,vi(k+1,im));
    ffor(i,0,k+1){dp[n][i]=0;}
    for(ll i = n - 1; i >= 0; i--){
    for(ll j = k; j >= 0; j--){
        for(ll r = i + 1; r <= n; r++){
            ll removed = r - i - 1;
            if(removed <= j){
                dp[i][j] = min(dp[i][j], 
                speed[i]*(coordinate[r]-coordinate[i])
                + dp[r][j - removed]);
            }
        }
    }}
