...
/*------------------------------------begin------------------------------------*/
int lastNode=-1;
void fun(int root,vvi&g,int lvl,int&mxLvl,int prnt){
    if(lvl>mxLvl){
        lastNode=root;
        mxLvl=lvl;
    }
    vfor(g[root]){
        if(*itr!=prnt)fun(*itr,g,lvl+1,mxLvl,root);
    }
}
...
// void bfs(queue<int>&q,vvi&g,int&lastNode,vi&ex){
//     if(q.empty())return;
//     int root=q.front();
//     ex[root]=1;
//     q.pop();
//     lastNode=root;
//     vfor(g[root]){
//         if(ex[*itr]==0)q.push(*itr);
//     }
//     bfs(q,g,lastNode,ex);
// }
//bfs is done for the fiorst time instead of dfs this is beacuse
//in normal dfs we cant gaurentee last encournted node is the diameter end point
//but in case of bfs we can 100% surely say last encountered node is our diameter end point!
//to do it with dfs you must have to remember the node with max lvl(i.e wht bfs was doing)
