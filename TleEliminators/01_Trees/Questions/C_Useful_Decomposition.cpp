...
void fun(int vrtx,vvi &g,int parent,vi&ans,int root){
    if(g[vrtx].sz==1&&vrtx!=root){
        ans.pb(vrtx);return;
    }
    vfor(g[vrtx]){
        if(*itr!=parent)fun(*itr,g,vrtx,ans,root);
    }
}

void solve()
{
    //each edge should be in exactly one path -> branching is not allowed!
    //every 2 paths must have a vertex common in them! -->
    //  says ki there exist 1 vertex such that its present in all the paths!
    in(v);
    vvi g(v+1,vi());
    ffor(i,0,v-1){
        in2(x,y);
