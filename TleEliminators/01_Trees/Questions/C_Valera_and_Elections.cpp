...
bool chk(int a,int b){
    int a1=a,b1=b;
    maxi(a1,b1);
    if(m[{a1,b1}]==2)return T;
    return F;

}
void dfs(int root,int prnt,vvi&g,int isProb,vi&ans){
    // pt2(root,isProb);
    vfor(g[root]){
        if(*itr==prnt)continue;
        if(chk(root,*itr)){
            if(isProb!=-1)ans[isProb]=0;
            ans[*itr]=1;
            dfs(*itr,root,g,*itr,ans);
        }
        else
            dfs(*itr,root,g,isProb,ans);
    }
}
