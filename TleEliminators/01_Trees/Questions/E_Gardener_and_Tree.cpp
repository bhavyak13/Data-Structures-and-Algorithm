...
void solve()
{
    in2(v,k);
    vvi g(v+1,vi());
    vi remainingNeighbours(v+1,0);
    vi layer(v+1,0);
    ffor(i,0,v-1){
        in2(x,y);
        g[x].pb(y);
        g[y].pb(x);
        remainingNeighbours[x]++;
        remainingNeighbours[y]++;
    }
    queue<int>q;
    ffor(i,1,v+1){
        if(g[i].sz==1){q.push(i);layer[i]=1;}
    }
    while(!q.empty()){
        int x=q.front();
        q.pop();
        vfor(g[x]){
            if(remainingNeighbours[*itr]<=1)continue;
            remainingNeighbours[*itr]--;
            if(remainingNeighbours[*itr]<=1){
                layer[*itr]=layer[x]+1;
                q.push(*itr);
            }
        }
