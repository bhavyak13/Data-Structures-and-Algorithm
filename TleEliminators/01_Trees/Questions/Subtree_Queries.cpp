...
int dfs(int root,int prnt,vvi&g,vi&val,vi&sum){
    int c=0;
    vfor(g[root]){
        if(*itr!=prnt){
            c+=dfs(*itr,root,g,val,sum);
        }
    }
    c+=val[root];
    return sum[root]=c;
}
//this is a segment tree + flatening a tree problem
//go and learn segment tree first!
void solve()
{
    in2(v,q);
    vi val(v+1);
    ffor(i,1,v+1)cin>>val[i];
    vvi g(v+1,vi());
    ffor(i,0,v-1){
        in2(x,y);
        g[x].pb(y);
    ...
    ffor(i,0,q){
        in(x);
        if(x==1){
            //change value of node a ,now a = b;
            in2(a,b);
        }else{
            in(a);
            pn(sum[a]);
