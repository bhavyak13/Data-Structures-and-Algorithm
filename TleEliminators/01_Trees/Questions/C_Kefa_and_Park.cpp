...
//hamesha graph.tree refernce se dena vrna TLE aane ke chnces rhngy!
int m,ans;
void fun(int vrtx,vvi&g,int currCat,vi&isCat,int parent){
    int cNew=currCat;
    if(isCat[vrtx]==1){cNew++;}
    else cNew=0;
    if(cNew>m){return;}
    vfor(g[vrtx]){
        if(*itr==parent)continue;
        fun(*itr,g,cNew,isCat,vrtx);
    }
    if(g[vrtx].sz==1&&vrtx!=1)ans++;
}
...
void solve()
{
    ...
    fun(1,g,0,a,-1);
    //more than m cats nhi honi chahie!
    pn(ans);
}
