...
//used for priority_queue (key increasing order )
...
struct SegTree{
    ...
    void build_tree(int l,int r,int indx){
        ...
        // pt4(l,r,tree[indx].val,tree[indx].minCount);
    }
...
struct Node1{
    ...
    void merge(Node1&l,Node1&r){ // l and r are child nodes
        ...
        if(l.val==r.val)minCount=l.minCount+r.minCount;
        else if(l.val<r.val)minCount=l.minCount;
        else minCount=r.minCount;
    }
};
struct Update1{
    int val; 
    Update1(int value){ //store the value which was recieved during update call
        val=value;
    }
...
void solve(){
    ...
    while(q--){
        ...
        if(t==1){
            ...
            auto it=seg.make_query(x,y-1);
            pt2(it.val,it.minCount);
        }
        // seg.fun(0,n-1,1);
        // cout<<endl;
