...
void build_tree(int indx,int l,int r){
    ...
    seg[indx]=max(seg[2*indx],seg[2*indx+1]);
}
int ans=0;
void query_update(int indx,int l,int r,int val){
    if(val>seg[indx])return;
    ...
    int mid=(l+r)/2;
    if(seg[2*indx]>=val){
        query_update(2*indx,l,mid,val);
    }else{
        query_update(2*indx+1,mid+1,r,val);
    }
    seg[indx]=max(seg[2*indx],seg[2*indx+1]);
}
/*------------------------------------begin------------------------------------*/
