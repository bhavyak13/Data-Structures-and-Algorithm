...
struct SegTree{
    ...
    bool qry(int indx,int l,int r,int qVal,int qIndx){
        if(tree[indx].val<qVal||r<qIndx){
            return false;
        }
        if(l==r){
            pn(l);
        ...
        int mid=(l+r)/2;
        if(tree[2*indx].val>=qVal&&mid>=qIndx){
            if(qry(2*indx,l,mid,qVal,qIndx))return T;
        }
        if(tree[2*indx+1].val>=qVal){
            if(qry(2*indx+1,mid+1,r,qVal,qIndx))return T;
        }
        return F;
    }
...
void solve()
{
    ...
    while(q--){
        ...
        if(t==2){
            in2(val,indx);
            if(!seg.qry(1,0,n-1,val,indx))pn(-1);
        }else{
            in2(indx,val);
            seg.make_update(indx,val);
        }
    }
}
