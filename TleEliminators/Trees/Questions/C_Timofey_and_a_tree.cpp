...
auto fun(int value,vp& possible){
    vfor(possible){
        if((*itr).ff!=value&&(*itr).ss!=value){return false;}
    }
    return true;
}
...
void solve()
{
    ...
    if(fun(possible[0].ff,possible)){
        pn(Y);pn(possible[0].ff);return;
    }
    if(fun(possible[0].ss,possible)){
        pn(Y);pn(possible[0].ss);return;
    }
    pn(N);
}
