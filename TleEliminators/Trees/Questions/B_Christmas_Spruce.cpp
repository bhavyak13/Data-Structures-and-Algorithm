...
/*------------------------------------begin------------------------------------*/
int c=0;
void fun(vector<vector<int>>g,vi &inTime,vi &outTime,int vertx){
    inTime[vertx]=c++;
    vfor(g[vertx]){
        fun(g,inTime,outTime,*i);
    }
    outTime[vertx]=c++;
}
...
void solve()
{
    ...
    fun(g,inTime,outTime,1);
    //check 3 leaf childern
    ffor(j,1,n+1){
        if(inTime[j]+1==outTime[j])continue;
        int cnt=0;
        vfor(g[j]){
            if(inTime[*i]+1==outTime[*i]){cnt++;}
        }
        if(cnt<3){
            pn("No");return;
        }
