...
int node,maxLvl;
void dfs(int root ,vvi& g,int prnt,int lvl){
    vfor(g[root]){
        if(*itr!=prnt){
            dfs(*itr,g,root,lvl+1);
        }
    }
    if(lvl>maxLvl){
        maxLvl=lvl;
...
void dfs2(int root,vvi&g,int prnt,int lvl,vi&ans){
    vfor(g[root]){
        if(*itr!=prnt){
            dfs2(*itr,g,root,lvl+1,ans);
        }
    }
    ans[root]=max(ans[root],lvl);
}
