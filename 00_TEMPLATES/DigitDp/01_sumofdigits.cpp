...
ques: find count of numbers between L and R which have a sum of digits = X
...
dp[numDig][sumOfDig][tight] -> count of numbers that have numDigits digits in it and whose sum of digits is 'sumOfDig' 

dp[numDig][sumOfDig][1] -> 
count of numbers that have 'numDigits' digits in it and whose sum of digits is 'sumOfDig' and we are under a tight constraint

dp[numDig][sumOfDig][0] -> 
count of numbers that have 'numDigits' digits in it and whose sum of digits is 'sumOfDig', and we aren't under a tight constraint
...
int fun(string& num,int numDigits,int sumOfDig,int tight){
    if(sumOfDig==0&&numDigits==0)return 1;
    if(numDigits==0)return 0;
    if(dp[numDigits][sumOfDig][tight]!=-1)return dp[numDigits][sumOfDig][tight];
    int rdig=tight?num[num.sz-numDigits]-'0' :9;
    int ans=0;
    for(int dig=0;dig<=rdig;dig++){
        int remSum=sumOfDig-dig;
        if(remSum>=0){
            ans+=fun(num,numDigits-1,remSum,(tight)&(dig==rdig));
        }
    }
    return dp[numDigits][sumOfDig][tight]=ans;
}
