...
void push_down(int l,int r,int indx){
    if(l==r){
        lazy[indx]=0;
        return;
    }
...
void update_tree(int indx, int l, int r, int qL,int qR, int qVal){
    if(lazy[indx]>0){
        seg[indx]+=lazy[indx]*(r-l+1);
        push_down(l,r,indx);
    }
    // completely lies
    if(l>=qL&&r<=qR){
    ...
    if(r<qL||l>qR)return;// disjoint
    int mid=(l+r)/2;
    update_tree(2*indx,l,mid,qL,qR,qVal);
    update_tree(2*indx+1,mid+1,r,qL,qR,qVal);
    seg[indx]=seg[2*indx]+seg[2*indx+1];
}
...
int query_tree(int indx,int l,int r,int ql,int qr){
    ...
    // So, our current node is updated!!
    if(l>=ql&&r<=qr){//completely lies
        return seg[indx];
    }
    if(l>qr||r<ql){//disjoint
        return 0;//identity
    }
    //overlaps
    int mid=(l+r)/2;
