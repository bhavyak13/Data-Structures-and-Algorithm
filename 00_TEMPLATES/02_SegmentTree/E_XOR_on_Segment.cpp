...
/*------------------------Lazy Generic Segment Tree Template begin----------------------------*/
...
void funMerge(vi &a,vi&b,vi &res){
    ffor(i,0,32){
        res[i]=a[i]+b[i];
    }
}
...
struct Update1 {
    ...
    void combine(Update1& new_update, int start, int end){
        xrVal ^= new_update.xrVal;
    }
};


/*------------------------Lazy Generic Segment Tree Template End----------------------------*/
...
void solve()
{
    ...
    while(q--){
        ...
        l--;r--;
        if(t==1){
            auto x=lst.make_query(l,r).bits;
            // for(auto i:x)pt(i);
            pn(calcValue(x));
        }else{
            in(X);
            lst.make_update(l,r,X);
        }
    }
